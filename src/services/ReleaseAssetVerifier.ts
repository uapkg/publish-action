...
export class ReleaseAssetVerifier {
  ...
  async verify(
    request: ReleaseAssetVerificationRequest
  ): Promise<Result<string>> {
    const bag = new DiagnosticBag()

    const releaseResult = await this.apiExecutor.execute(
      `get release by tag ${request.releaseTag}`,
      () =>
    ...
    const matchedAssets = releaseResult.value.data.assets.filter((asset) =>
